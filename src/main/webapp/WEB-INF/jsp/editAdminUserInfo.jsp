...
//            var pwd=document.getElementById("user_password").value.length;
    ...
    <title>${adminUser.adminName }&nbsp;信息编辑</title>
</head>
...
<div class="reg_page">
    ...
    <form name="Register_form" method="post" enctype="multipart/form-data"
          action="${pageContext.request.contextPath }/admin/editadminusersubmit.su">

        <input name="adminHeader_default" type="hidden" value="${adminUser.adminHeader_default }"/>
        <input name="adminLoginIp" type="hidden" value="${adminUser.adminLoginIp }">
        <input name="adminLoginTime" type="hidden" value="${adminUser.adminLoginTime }">
        <input name="adminID" type="hidden" value="${adminUser.adminID }">
        <div class="reg_content">
            <ul>
                <li><input name="adminID" disabled type="text" value="${adminUser.adminID }"/></li>
                <li><input name="adminName" type="text" value="${adminUser.adminName }"/></li>

                <li class="radio_ctrl">
                    <%--登录的如果是root 用户--%>
                    <c:if test="${admin.adminPermissions eq 'root' }">
                        <%-- root 用户不能被降低权限，其他用户不能被升级为root，因为只能存在一个--%>
                        <c:if test="${adminUser.adminPermissions eq 'root' }">
                            <input checked class="radio_ctrl_in" name="adminPermissions" type="radio" value="root">超级管理员
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio"
                                   value="administrator">管理员
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio" value="general">审阅员
                        </c:if>
                        <c:if test="${adminUser.adminPermissions eq 'administrator' }">
                            <input class="radio_ctrl_in" name="adminPermissions" type="radio" value="root">超级管理员
                            <input checked class="radio_ctrl_in" name="adminPermissions" type="radio" value="administrator">管理员
                            <input class="radio_ctrl_in" name="adminPermissions" type="radio" value="general">审阅员
                        </c:if>
                        <c:if test="${adminUser.adminPermissions eq 'general' }">
                            <input class="radio_ctrl_in" name="adminPermissions" type="radio" value="root">超级管理员
                            <input class="radio_ctrl_in" name="adminPermissions" type="radio" value="administrator">管理员
                            <input checked class="radio_ctrl_in" name="adminPermissions" type="radio" value="general">审阅员
                        </c:if>
                    </c:if>
                    <%--登录的如果不是root 用户，此页面显示只读状态--%>
                    <c:if test="${admin.adminPermissions ne 'root' }">
                        <c:if test="${adminUser.adminPermissions eq 'administrator' }">
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio" value="root">超级管理员
                            <input readonly checked class="radio_ctrl_in" name="adminPermissions" type="radio" value="administrator">管理员
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio" value="general">审阅员
                        </c:if>
                        <c:if test="${adminUser.adminPermissions eq 'general' }">
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio" value="root">超级管理员
                            <input disabled class="radio_ctrl_in" name="adminPermissions" type="radio" value="administrator">管理员
                            <input readonly checked class="radio_ctrl_in" name="adminPermissions" type="radio" value="general">审阅员
                        </c:if>
                    </c:if>
                </li>

                <li><input name="adminPassword" type="password" value="${adminUser.adminPassword }"></li>
                <li><input name="adminPassword_again" type="password" value="${adminUser.adminPassword }"></li>

                <li><input name="adminPhoneNumber" type="tel" value="${adminUser.adminPhoneNumber }"
                           required pattern="^(0|86|17951)?(13[0-9]|15[012356789]|17[0678]|18[0-9]|14[57])[0-9]{8}">
                </li>
                <%--spellcheck="false" :关闭字符修正提示--%>
                <li><input name="adminEMail" type="email" value="${adminUser.adminEMail }" spellcheck="false"></li>
                <li><input name="adminNote" type="text" value="${adminUser.adminNote }"></li>
                <li>
